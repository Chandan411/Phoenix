...
function nextInvoiceNumber() {
  const txn = db.transaction(() => {
    const row = db.prepare('SELECT last_seq FROM invoice_seq WHERE id = 1').get();
...
function normalizeGstFields(items, gstNumber) {
  ...
  return items.map(it => {
    if (isCgstSgst) {
      ...
      let cgst = it.cgst_rate !== undefined ? Number(it.cgst_rate) : (it.igst_rate ? Number(it.igst_rate) / 2 : 0);
      let sgst = it.sgst_rate !== undefined ? Number(it.sgst_rate) : (it.igst_rate ? Number(it.igst_rate) / 2 : 0);
      return { ...it, cgst_rate: cgst, sgst_rate: sgst, igst_rate: 0 };
    } else {
      // If igst missing but cgst/sgst present, sum them
      let igst = it.igst_rate !== undefined ? Number(it.igst_rate) : ((Number(it.cgst_rate) || 0) + (Number(it.sgst_rate) || 0));
...
router.post('/', async (req, res) => {
  try {
    ...
    if (!body.customer_name || !Array.isArray(body.items) || body.items.length === 0) {
      return res.status(400).json({ error: 'Missing customer_name or items[]' });
    }
    const invoiceNumber = body.invoice_number && typeof body.invoice_number === 'string'
      ? body.invoice_number
      : nextInvoiceNumber();
...
router.get('/:id/pdf', (req, res) => {
  try {
    ...
    if (!row || !row.file_path) return res.status(404).json({ error: 'PDF not found for invoice id' });
...
router.get('/', (req, res) => {
  try {
    const rows = db.prepare('SELECT * FROM invoices ORDER BY invoice_date DESC, id DESC').all();
...
router.put('/:id', async (req, res) => {
  const id = Number(req.params.id);
  const { invoice_date, customer_name, customer_address, customer_gst, items } = req.body;
  try {
    const normalizedItems = normalizeGstFields(items, customer_gst || body.customer_gst);
    const calc = calculateTotals(normalizedItems);
    db.prepare(
...
router.get('/product/:product_name', (req, res) => {
  ...
  try {
    const product = db.prepare('SELECT * FROM products WHERE product_name = ?').get(product_name);
